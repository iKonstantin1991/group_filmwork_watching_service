...
from tests.conftest import build_headers, get_random_user
...
def test_get_place_by_place_id_correctly(user: User) -> None:
    ...
    response = httpx.get(f"{settings.service_url}/api/v1/places/{created_place_id}", headers=build_headers(user.token))
    assert response.status_code == HTTPStatus.OK
    _assert_place(response.json(), expected_place)
...
def test_get_places_by_host_correctly() -> None:
    ...
    response = httpx.post(
        ...
        headers=build_headers(user_one.token),
    )
    ...
    created_place = httpx.post(
        ...
        headers=build_headers(user_two.token),
    )
    _assert_place(created_place.json(), expected_place_two)

    response = httpx.get(
        f"{settings.service_url}/api/v1/places/?host_id={user_one.id}", headers=build_headers(user_one.token)
    )
    assert response.status_code == HTTPStatus.OK
